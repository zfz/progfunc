...
class Empty extends TweetSet {
  ...
  /* Part three */
  def mostRetweeted: Tweet = throw new NoSuchElementException("empty Tweets")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  /* Part two */
  def union(that: TweetSet): TweetSet = {
    /* The first will cause endless problem.*/
  ...
  /* Part three */
  def mostRetweeted: Tweet = {
    def compareRetweet(s: Tweet, t: Tweet): Tweet = {
      if (s.retweets > t.retweets) s
      else t
    }
    ...
    if (left.isEmpty && right.isEmpty) {
      ...
      compareRetweet(right.mostRetweeted, elem)
    }
    else if (right.isEmpty) {
      compareRetweet(left.mostRetweeted, elem)
    }
    else {
      val leaveMostRetweeted = compareRetweet(left.mostRetweeted, 
        right.mostRetweeted)
      compareRetweet(leaveMostRetweeted, elem)
    }
  }
